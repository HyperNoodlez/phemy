...
private let phemyLog = Logger(subsystem: "com.labgarge.phemy", category: "FFI")

/// Swift wrapper around the Rust phemy-core C FFI.
/// All complex types are exchanged as JSON strings over the C boundary.
final class PhemyCore {
    static let shared = PhemyCore()
    ...
    private init() {
        ...
        let dataDir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
            .first!
            .appendingPathComponent("com.labgarge.phemy")
            .path
    ...
    private func consumeRustString(_ ptr: UnsafeMutablePointer<CChar>?) -> String? {
        ...
        guard let str = String(validatingUTF8: ptr) else {
            phemyLog.warning("Rust FFI returned invalid UTF-8 data")
            phemy_free_string(ptr)
            return nil
        }
        phemy_free_string(ptr)
        return str
    ...
    private func decodeRustJSON<T: Decodable>(_ ptr: UnsafeMutablePointer<CChar>?, as type: T.Type) -> T? {
        ...
        do {
            ...
            phemyLog.warning("Failed to decode \(String(describing: type)) from Rust JSON: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func saveSettings(_ settings: AppSettings) {
        ...
        json.withCString { ptr in
            _ = phemy_save_settings(ptr)
        }
    }

    func resetSettings() -> AppSettings {
        let ptr = phemy_reset_settings()
        return decodeRustJSON(ptr, as: AppSettings.self) ?? .default
    }
    ...
    func listAudioDevices() -> [AudioDeviceInfo] {
        let ptr = phemy_list_audio_devices()
    ...
    func startRecording(device: String? = nil) -> Bool {
        if let device = device {
            return device.withCString { ptr in
                phemy_start_recording(ptr, phemyMicLevelCallback)
            }
        } else {
            return phemy_start_recording(nil, phemyMicLevelCallback)
        }
    }

    func stopRecording() -> String? {
        consumeRustString(phemy_stop_recording())
    }
    ...
    func stopAndProcess() -> StopAndProcessResult {
        let ptr = phemy_stop_and_process()
        guard let json = consumeRustString(ptr),
